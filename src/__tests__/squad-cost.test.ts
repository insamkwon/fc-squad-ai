 ...
 *   3. formatPrice handles 경/조/억/만 notation
...
describe('formatCost', () => {
  ...
  it('formats amounts >= 1억 in 억 notation with BP suffix', () => {
    expect(formatCost(100_000_000)).toBe('1억 BP');     // exactly 1억
    expect(formatCost(500_000_000)).toBe('5억 BP');     // 5억
    expect(formatCost(1_000_000_000)).toBe('10억 BP');  // 10억
    expect(formatCost(1_500_000_000)).toBe('15억 BP');  // 15억
    expect(formatCost(52_000_000_000)).toBe('520억 BP'); // 520억
  });

  it('formats amounts < 1억 in 만 notation with BP suffix', () => {
    expect(formatCost(50_000_000)).toBe('5,000만 BP');     // 5000만
    expect(formatCost(10_000_000)).toBe('1,000만 BP');     // 1000만
  ...
  it('handles large squad totals correctly', () => {
    expect(formatCost(16_500_000_000)).toBe('165억 BP');
  });
  ...
  it('formats 조-range squad totals', () => {
    expect(formatCost(1_000_000_000_000)).toBe('1조 BP');
    expect(formatCost(310_000_000_000_000)).toBe('310조 BP');
  });

  it('formats 경-range squad totals', () => {
    expect(formatCost(15_100_000_000_000_000)).toBe('15경 100조 BP');
  });
});
...
describe('formatPrice', () => {
  it('formats prices >= 1억 without decimal when clean', () => {
    expect(formatPrice(5_200_000_000)).toBe('52억');
    expect(formatPrice(1_000_000_000)).toBe('10억');
    expect(formatPrice(1_500_000_000)).toBe('15억');
  });

  it('formats prices < 1억 in 만 units', () => {
    expect(formatPrice(35_000_000)).toBe('3,500만');
  ...
  it('handles edge cases', () => {
    ...
    expect(formatPrice(100_000_000)).toBe('1억');
  });

  it('formats 조-range prices', () => {
    expect(formatPrice(310_000_000_000_000)).toBe('310조');
    expect(formatPrice(242_000_000_000_000)).toBe('242조');
    expect(formatPrice(51_000_000_000_000)).toBe('51조');
  });

  it('formats 경-range prices', () => {
    expect(formatPrice(15_100_000_000_000_000)).toBe('15경 100조');
    expect(formatPrice(1_510_000_000_000_000)).toBe('1경 510조');
  });
});
...
describe('Squad cost integration', () => {
  it('calculates and formats a realistic squad total', () => {
    const players = [
      { player: { price: 8_000_000_000 } },
      { player: { price: 5_500_000_000 } },
      { player: { price: 3_000_000_000 } },
      { player: { price: 4_200_000_000 } },
      { player: { price: 2_800_000_000 } },
      { player: { price: 6_000_000_000 } },
      { player: { price: 3_500_000_000 } },
      { player: { price: 7_000_000_000 } },
      { player: { price: 6_500_000_000 } },
      { player: { price: 4_000_000_000 } },
      { player: { price: 2_000_000_000 } },
    ];

    const total = calculateSquadCost(players);
    expect(total).toBe(52_500_000_000);

    const formatted = formatCost(total);
    expect(formatted).toBe('525억 BP');
  });

  it('formats a budget squad total correctly', () => {
    const players = Array.from({ length: 11 }, () => ({
      player: { price: 45_000_000 },
    }));

    const total = calculateSquadCost(players);
    expect(total).toBe(495_000_000);

    const formatted = formatCost(total);
    expect(formatted).toBe('4.9억 BP');
  });
});
